...
label_0 = 0 # All cat images are labelled 0
label_1 = 1 # All dog images are labelled 1
...
def separate_img_label(data):
    imgs = []
    img_labels = []
    print(f"Data Labels: {data.class_indices}")
    for (file, label) in zip(data.filepaths, data.labels):
        try:
            imgs.append(plt.imread(file))
            img_labels.append(label)
        except IOError:
            print(f"{file} is corrupt. Removing file.")
            os.remove(file)
            pass
    if len(imgs) == len(img_labels):
        return (imgs, img_labels)
    else:
        print("Image array and Labels are not of same size. Please check the Keras Data Flow Generator process.")
...
def load_data(data_dir="./data"):
    ...
    train_data = data_gen.flow_from_directory(directory=data_dir, color_mode='grayscale', target_size=(28, 28), class_mode='binary', seed=5, subset='training')
    test_data = data_gen.flow_from_directory(directory=data_dir, color_mode='grayscale', target_size=(28, 28), class_mode='binary', seed=5, subset='validation')
    (train_images, train_labels) = separate_img_label(train_data)
    (test_images, test_labels) = separate_img_label(test_data)
    train_ctr_0 = 0
    test_ctr_0 = 0
    train_ctr_1 = 0
    test_ctr_1 = 0
