...
def download_dataset(url=ZIP_URL, zip_dest=ZIP_PATH, data_path=DATA_PATH):
    """
    Download the cats and dogs dataset (Asirra CAPTCHA dataset) from Microsoft
    Arguments:
        url: URL pointing to the downloadable zip file of images
        zip_dest: Download destination for the zip file
        data_path: File path to extract the zip file into
    Returns:
        None
    """
...
def extract_data(zip_path=ZIP_PATH, dest_path=DATA_PATH):
    ...
    Arguments:
        zip_path: Local path of the Asirra CAPTCHA dataset zip file
        dest_path: File path to extract the zip file into
    Returns:
        None
    """
...
def check_and_download(url=ZIP_URL, zip_path=ZIP_PATH, data_path=DATA_PATH):
    """
    Check if the extracted files exist. Download and extract if files not found.
    Arguments:
        url: URL pointing to the downloadable zip file of images
        zip_path: Download destination for the zip file
        data_path: File path to extract the zip file into
    Returns:
        None
    """
...
if __name__ == "__main__":
    check_and_download(url=ZIP_URL, zip_path=ZIP_PATH, data_path=DATA_PATH)
